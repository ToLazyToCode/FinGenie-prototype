...
function PiggyBankSimulator() {
    ...
    // Tính toán xem mức chi tiêu tối thiểu đặt ra có hợp lý không
    const checkOverMinSaving = () => {
        let avgSpending = calculateDailyAvgSpending();
        return minSaving - avgSpending;
    }
    ...
    const runSimulation = (e) => {
        ...
        if (checkOverMinSaving() > 0) {
            console.log(minSaving);
            console.log(calculateDailyAvgSpending());
            showToast("Mức tiết kiệm tối thiểu cao hơn so với mức chi tiêu trung bình hàng ngày\n" +
                `${(minSaving - calculateDailyAvgSpending()).toLocaleString('vi-VN')} VNĐ`, "error");
            return;
        }
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-lg">
            ...
            <p className="text-sm text-slate-500 mb-4">Hãy thử tương tác với logic MVP cốt lõi của chúng tôi. Chọn một
                chế độ và xem AI của FinGenie gợi ý tiết kiệm như thế nào.</p>

            <form className="space-y-4" onSubmit={runSimulation}>
                <div>
                    <label htmlFor="monthlyIncome" className="block text-sm font-medium text-slate-700">Thu nhập hàng
                        tháng (VNĐ)</label>
                    <input
                        type="number"
                        id="monthlyIncome"
                ...
                <div>
                    <label htmlFor="spendingRatio" className="block text-sm font-medium text-slate-700">Tỉ lệ chi tiêu
                        trung bình (0-1)</label>
                    <input
                        type="number"
                        id="spendingRatio"
                        className="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500"
                ...
                <div>
                    <label htmlFor="monthlySavingTarget" className="block text-sm font-medium text-slate-700">Mục tiêu
                        tiết kiệm tháng (VNĐ)</label>
                    <input
                        type="number"
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label htmlFor="currentDay" className="block text-sm font-medium text-slate-700">Ngày hiện tại
                            (1-30)</label>
                        <input
                            type="number"
                            id="currentDay"
                ...
                <div>
                    <label htmlFor="spending" className="block text-sm font-medium text-slate-700">Chi tiêu trong ngày
                        (VNĐ)</label>
                    <input
                        type="number"
                        id="spending"
                ...
                {savingMode === 'flexible' ? (
                    <div className="space-y-4">
                        <div>
                            <label htmlFor="formulaType" className="block text-sm font-medium text-slate-700">Loại công
                        ...
                        <div>
                            <label htmlFor="minSaving" className="block text-sm font-medium text-slate-700">Tiết kiệm
                                Tối thiểu (VNĐ)</label>
                            <input
                                type="number"
                                id="minSaving"
                        ...
                        <div>
                            <label htmlFor="maxSaving" className="block text-sm font-medium text-slate-700">Tiết kiệm
                                Tối đa (VNĐ)</label>
                            <input
                                type="number"
                                id="maxSaving"
                            ...
                            <div>
                                ...
                                <ul className="list-disc pl-5 space-y-1 text-sm mt-2">
                                    <li>Chi tiêu hợp lý cho các ngày còn
                                        lại: <strong>{Math.round(calculateRebalancePlan().targetDailySpending).toLocaleString('vi-VN')} VNĐ/ngày</strong>
                                    </li>
                                    {calculateRebalancePlan().isBudgetExceeded && (
                                        <li className="text-red-600">
                                            Cảnh báo: Bạn đã vượt ngân
                                            sách {Math.abs(calculateRebalancePlan().remainingBudget).toLocaleString('vi-VN')} VNĐ
                                        </li>
                                    )}
                                </ul>
                            </div>
